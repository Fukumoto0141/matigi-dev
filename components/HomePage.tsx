...
export function HomePage() {
  ...
  const stats = [
    { icon: Users, label: "議員数", value: "35名", color: "#4DA9E8" },
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {stats.map((stat, index) => {
          ...
          return (
            <div
              key={index}
              className="bg-white rounded-xl p-4 shadow-sm border border-[#E1E8ED]"
            >
      ...
      <div className="bg-white rounded-xl p-6 shadow-sm border border-[#E1E8ED]">
        ...
        <div className="space-y-3">
          {recentBills.map((bill) => (
            <div
              key={bill.id}
              className="flex items-center justify-between p-4 bg-[#F5F7FA] rounded-lg hover:bg-[#E8EDF2] transition-colors cursor-pointer"
            >
